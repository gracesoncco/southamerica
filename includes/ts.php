...
function ts_gfx($ts_random) {
	...
	$datekey = date('F j');
	$rcode = md5($_SERVER['HTTP_USER_AGENT'] . $site_key . $ts_random . $datekey);
	
	$code = substr($rcode, 4, 6);
	$circles=5;
	$lines=0;
	$width=100;
	$height=25;
...
#rgb(1%, 51%, 87%)
	...
	$start = 5;
	for ($i=0;$i<$count;$i++) {
		$start += rand(0, 5) + $fontwidth;
		imagechar($im, $font, $start, rand(0, $height - $fontheight), 
				rand(0, 1) ? $code[$i] : strtoupper($code[$i]), 
				$text_color);
	}
...
function ts_is_human() {
	...
	if (empty($_POST['ts_code']) || empty($_POST['ts_random']))
		return false;
		
	$ts_code = strtolower(trim($_POST['ts_code']));
	$ts_random = trim($_POST['ts_random']);
	$datekey = date("F j");
	$rcode = md5($_SERVER['HTTP_USER_AGENT'] . $site_key . $ts_random . $datekey);
	$code = substr($rcode, 4, 6);
